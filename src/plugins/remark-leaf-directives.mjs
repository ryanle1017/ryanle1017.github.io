...
const embedHandlers = {
  ...
  // Netease Cloud Music (APlayer)
  music: (node) => {
    const id = node.attributes?.id ?? ''
    if (!id) {
      console.warn(`Missing Netease music ID`)
      return false
    }
    ...
    return `
    <div class="aplayer-wrap" data-id="${id}" data-title="${title}" data-artist="${artist}" data-cover="${cover}"></div>
    `
  },
